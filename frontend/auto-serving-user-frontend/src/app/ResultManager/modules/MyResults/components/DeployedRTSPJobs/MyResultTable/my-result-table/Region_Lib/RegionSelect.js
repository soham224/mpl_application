...
export class RegionSelect extends Component {
	...
	onDocMouseTouchMove (event) {
		...
		if (!regionChangeData.isMove) {
			...
			x = Math.min(x1Pc, x2Pc)/this.props.scale;
			y = Math.min(y1Pc, y2Pc)/this.props.scale;
			width = Math.abs(x1Pc - x2Pc)/this.props.scale;
			height = Math.abs(y1Pc - y2Pc)/this.props.scale;
			if(this.props.constraint){
				if (x2Pc/this.props.scale >= 100) { x = x1Pc/this.props.scale; width = 100 - x1Pc/this.props.scale; }
				if (y2Pc/this.props.scale >= 100) { y = y1Pc/this.props.scale; height = 100 - y1Pc/this.props.scale; }
				if (x2Pc/this.props.scale <= 0) { x = 0; width = x1Pc/this.props.scale; }
				if (y2Pc/this.props.scale <= 0) { y = 0; height = y1Pc/this.props.scale; }
			}
		} else {
			x = ((clientPos.x + regionChangeData.clientPosXOffset - regionChangeData.imageOffsetLeft) / regionChangeData.imageWidth * 100) /this.props.scale;
			y = ((clientPos.y + regionChangeData.clientPosYOffset - regionChangeData.imageOffsetTop) / regionChangeData.imageHeight * 100) / this.props.scale;
	...
	onComponentMouseTouchDown (event) {
		...
		const rect = {
			x: xPc/this.props.scale,
			y: yPc/this.props.scale,
			width: 0,
			height: 0,
			new: true,
	...
	onRegionMoveStart (event, index) {
		...
		const regionLeft = ((currentRegion.x*this.props.scale) / 100 * this.refs.image.offsetWidth) + imageOffset.left;
		const regionTop = ((currentRegion.y*this.props.scale) / 100 * this.refs.image.offsetHeight) + imageOffset.top;
		const regionWidth = ((currentRegion.width*this.props.scale) / 100 * this.refs.image.offsetWidth);
		const regionHeight = ((currentRegion.height*this.props.scale) / 100 * this.refs.image.offsetHeight);
