...
router = APIRouter()
logging = MyLogger().get_logger("notification_api")
...
def add_notification(
    ...
    notification_details.created_date = datetime.datetime.utcnow().replace(
        microsecond=0
    )
    notification_details.updated_date = datetime.datetime.utcnow().replace(
        microsecond=0
    )
...
@router.get(
    "/get_all_notification_of_camera_supervisor",
    response_model=Page[schemas.NotificationRead],
)
def get_all_notification_of_camera_supervisor(
    db: Session = Depends(deps.get_db),
    ...
    try:
        ...
        notification_list = (
            crud.notification_crud_obj.get_all_notification_by_notification_type(
                db=db, user_id=user_id, notification_type="camera"
            )
        )
        if not notification_list:
            return paginate([])
        return paginate(notification_list)
    except Exception as e:
        raise HTTPException(status_code=500, detail="No Notification Data Found")
...
@router.get(
    "/get_notification_data_for_current_user_by_date",
    response_model=Page[schemas.NotificationRead],
)
def get_notification_data_of_supervisor_by_date(
    ...
    try:
        if crud.user.is_supervisor(current_user):
            ...
            if company_admin:
                ...
                return paginate([])
            notification_list = (
                crud.notification_crud_obj.get_notification_by_date_notification_type(
                    db=db,
                    start_date=start_date,
                    end_date=end_date,
                    user_id=user_id,
            ...
            user_id = current_user.id
            notification_list = crud.notification_crud_obj.get_notification_by_date(
                db=db,
                start_date=start_date,
                end_date=end_date,
                user_id=user_id,
        ...
        logging.info("get_notification_data_of_supervisor_by_date {}".format(ex))
        raise HTTPException(status_code=500, detail="No Notification Data Found")


@router.get(
    "/get_all_unread_notification_for_current_user",
    response_model=List[schemas.NotificationRead],
)
def get_all_unread_notification_for_current_user(
    db: Session = Depends(deps.get_db),
    current_user: models.User = Depends(deps.get_current_active_user),
) -> Any:
    try:
        if crud.user.is_supervisor(current_user):
            ...
            if company_admin:
                ...
                return []
            notification_list = (
                crud.notification_crud_obj.get_unread_notification_by_notification_type(
                    db=db, user_id=user_id, notification_type=["camera"]
                )
            )
        else:
        ...
        logging.info("get_all_unread_notification_for_current_user {}".format(ex))
        raise HTTPException(status_code=500, detail="No Notification Data Found")
